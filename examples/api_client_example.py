#!/usr/bin/env python3
"""
CHOMBEZA - Advanced Bug Bounty Scanner with ML False Positive Reduction
"""
...
try:
    from core.state import StateManager, ScanState, get_state_manager
    HAS_STATE = True
except ImportError:
    HAS_STATE = False
    logging.warning("State management module not available")

try:
    from core.ml_fp_reducer import MLFalsePositiveReducer, get_ml_reducer
    HAS_ML = True
except ImportError:
    HAS_ML = False
    logging.warning("ML false positive reducer not available")
...
try:
    ...
    HAS_CHECKERS = False
    logging.warning("Modular checkers not available")
...
# Smart parameter mapping - maps parameter name patterns to vulnerability types
PARAM_TYPE_MAP = {
    # IDOR / Access Control
    r'id|uid|user_id|account|profile|customer|order|transaction': ['idor', 'sqli', 'nosqli'],
    
    # Search parameters
    r'search|q|query|keyword|find|lookup': ['xss', 'sqli', 'ssti', 'ldapi'],
    
    # Redirect parameters
    r'redirect|next|url|path|return|return_to|redirect_uri|callback|goto': ['open_redirect', 'lfi', 'ssrf', 'crlf'],
    
    # File parameters
    r'file|document|download|path|filename|attachment': ['lfi', 'path_traversal', 'rce'],
    
    # API parameters
    r'api_key|apikey|token|access_token|jwt|bearer': ['jwt', 'cors'],
    
    # Form/Input parameters
    r'name|email|phone|address|comment|message': ['xss', 'sqli', 'ssti'],
    
    # Debug parameters
    r'debug|test|env|environment|config': ['info_disclosure', 'rce'],
    
    # Pagination parameters
    r'page|limit|offset|start|end': ['sqli', 'nosqli'],
    
    # Upload parameters
    r'upload|file|image|avatar|photo': ['rce', 'xxe', 'lfi'],
    
    # XML parameters
    r'xml|soap|data': ['xxe', 'xpathi'],
    
    # JSON parameters
    r'json|data|payload': ['nosqli', 'prototype_pollution', 'deserialization'],
    
    # GraphQL parameters
    r'query|mutation|variables': ['graphql'],
    
    # WebSocket parameters
    r'ws|wss|socket|websocket': ['websocket'],
    
    # Cloud metadata
    r'metadata|meta': ['cloud_metadata', 'ssrf'],
    
    # Default (catch-all)
    r'.*': ['xss', 'sqli', 'ssti']  # Conservative default
}
...
class Vulnerability:
    ...
    def __init__(self, name: str, severity: str, url: str, description: str, 
                 ...
                 ml_confidence: float = None, ml_classification: str = None):
        self.name = name
    ...
    def from_dict(cls, data: Dict) -> 'Vulnerability':
        ...
        vuln = cls(
            ...
            request_response=data.get('request_response'),
            ml_confidence=data.get('ml_confidence'),
            ml_classification=data.get('ml_classification')
        )
        vuln.cwe_id = data.get('cwe_id', 'CWE-Unknown')
        vuln.timestamp = data.get('timestamp', time.time())
        vuln.request_id = data.get('request_id', str(uuid.uuid4())[:8])
        vuln.screenshot = data.get('screenshot')
        vuln.screenshots = data.get('screenshots', [])
        return vuln
...
class Scanner:
    def __init__(self, config_path: str = None):
        ...
        self.cache = get_cache(self.config.get("cache_size", 1000)) if HAS_CACHE else None
        self.state_manager = get_state_manager() if HAS_STATE else None
        self.ml_reducer = get_ml_reducer() if HAS_ML else None
        ...
        if HAS_ML:
            logger.info("ML false positive reduction enabled")
        if HAS_CACHE:
            logger.info("Response caching enabled")
        if HAS_STATE:
            logger.info("Scan state persistence enabled")
    ...
    def set_scan_type(self, scan_type: str):
        ...
        # Update config with scan-specific settings
    ...
    def get_parameter_type(self, param_name: str) -> List[str]:
        """Determine vulnerability types based on parameter name"""
        param_lower = param_name.lower()
        
        for pattern, vuln_types in PARAM_TYPE_MAP.items():
            import re
            if re.search(pattern, param_lower):
                return vuln_types
        
        return PARAM_TYPE_MAP['.*']  # Default
    ...
    def _process_task(self, task: Dict, session: requests.Session):
        """Process a single scan task with caching"""
        url = task["url"]
        ...
        if self.cache:
            ...
            if cached_response:
                with self.stats_lock:
                    self.stats["cached"] += 1
                self._analyze_response(url, cached_response, vuln_type, params, data, 
                                      original_value, 0, request_id, cached=True)
                return
        ...
        try:
            ...
            if self.cache and 200 <= resp.status_code < 400:
                self.cache.set(cache_key, resp)
            
            self._analyze_response(url, resp, vuln_type, params, data, original_value, 
                                  response_time, request_id)
            
            # Checkpoint progress
            if self.completed_tasks % self.checkpoint_interval == 0:
                self._save_state()
    ...
    def _analyze_response(self, url: str, resp: requests.Response, vuln_type: str,
                         ...
                         response_time: float, request_id: str, cached: bool = False):
        ...
        if cached:
            request_response += "(Cached Response)\n"
    ...
    def _add_vulnerability(self, name: str, severity: str, url: str, description: str, 
        ...
        if HAS_ML and self.ml_reducer and self.ml_reducer.is_trained:
            ...
            probability, classification = self.ml_reducer.predict(finding_dict)
    ...
    def _discover_and_queue(self):
        ...
        try:
            ...
            mutations_per_payload = scan_config.get("mutations_per_payload", 
            ...
            for param in params:
                ...
                if self.config.get("smart_fuzz", True):
                    ...
                    vuln_types_to_test = enabled_vulns
                ...
                for vuln_type in vuln_types_to_test:
                    ...
                    for payload in payloads:
                        mutations = PayloadGenerator.mutate_payload(
                            payload, 
    ...
    def load_state(self, state_file: str) -> bool:
        ...
        if not HAS_STATE or not self.state_manager:
            logger.error("State management not available")
            return False
        
        state = self.state_manager.load_state(state_file)
        if not state:
            return False
        
        self.target = state.target
        self.scan_type = state.scan_type
        self.start_time = state.start_time
        self.completed_tasks = state.completed_tasks
        self.total_tasks = state.total_tasks
        self.stats = state.stats
        self.scanned_urls = set(state.scanned_urls)
        self.config.update(state.config)
        self.state_file = state_file
        
        # Recreate vulnerabilities
        self.vulnerabilities = [Vulnerability.from_dict(v) for v in state.vulnerabilities]
        
        logger.info(f"Loaded scan state from {state_file}")
        return True
    ...
    def generate_report(self) -> Dict:
        ...
        report_data = {
            ...
            "ml_stats": self.ml_reducer.get_stats() if HAS_ML and self.ml_reducer else None
        }
